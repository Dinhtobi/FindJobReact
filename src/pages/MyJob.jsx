...
const MyJob = () => {
    ...
    const  handleCancel = async (id) =>{
        ...
        .then(result => {
            alert("Bạn đã huỷ ứng tuyển!");
            
        })
        .catch(error => {
            console.warn("Huỷ thất bại");
            throw error;
        });
        setIsLoading(true);
        await getMyJob(user.token)
    ...
    return (
        <div className='max-w-full-2xl container mx-auto xl:px-24 px-24 py-8'>
            <div className='my-jobs-container'>
                <h1 className='text-center p4 font-semibold'>Job của tôi</h1>
                                    ...
                                    : ( <tbody>
                                        {
                                            currentJobs.map((job, index) => (
                                                ...
                                                <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                                                    <button><Link to={`/employeer/edit-job/${job?.id}`}>Edit</Link></button>
                                                </td>
                                                <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                                    ...
                                    : ( <tbody>
                                        {
                                            currentJobs.map((job, index) => (
                                                ...
                                                <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                                                    <button><Link to={`/seeker/${job?.id}`}>Xem</Link></button>
                                                </td>
                                                <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                                                    <button onClick={() => handleCancel(job.id)} className='bg-red-700 py-2 px-6 text-white rounded-sm'>Xoá</button>
                                                </td>
                                            </tr>
                                            ))
                                        }
